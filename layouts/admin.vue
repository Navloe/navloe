  ...
  <div v-if="!isLoading">
    <nav>
      <div class="flex items-center">
        ...
        <h2 class="font-semibold">Navloe Admin</h2>
      </div>
      <div class="relative">
    ...
    <fwb-sidebar class="transform sm:translate-x-0 top-16" :class="!showSidenav ? 'max-sm:-translate-x-full' : '!translate-x-0 max-w-[100%-44px]'">
      <fwb-sidebar-item link="/admin/dashboard" class="!font-normal !text-sm" @click="showSidenav = false">
        <template #icon>
          <Icon name="material-symbols:dashboard" color="#1560BD77" class="text-xl" />
        </template>
        <template #default>Dashboard</template>
      </fwb-sidebar-item>
      <fwb-sidebar-item link="/admin/user" class="!font-normal !text-sm" @click="showSidenav = false">
        <template #icon>
          <Icon name="material-symbols:person" color="#1560BD77" class="text-xl" />
        </template>
      ...
      <fwb-sidebar-dropdown-item class="!font-normal !text-sm">
        ...
        <template #trigger> <span class="!text-sm">UMKM</span> </template>
        <template #default>
          <fwb-sidebar-item class="pl-11" link="/admin/umkm" @click="showSidenav = false"> Data UMKM </fwb-sidebar-item>
          <fwb-sidebar-item class="pl-11" link="/admin/umkm/pengajuan" @click="showSidenav = false"> Pengajuan UMKM </fwb-sidebar-item>
        </template>
