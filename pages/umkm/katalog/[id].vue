<template>
  <div>
    <h1 class="mb-2 font-bold text-xl">Edit Katalog</h1>
        ...
        <div class="my-8">
          <div class="grid grid-cols-2 gap-6">
            <div class="flex items-center p-2 border border-gray-200 rounded dark:border-gray-700">
              <fwb-radio v-model="catalogs.status" label="Tampilkan" name="radio-bordered" value="show" />
            </div>
            <div class="flex items-center p-2 border border-gray-200 rounded dark:border-gray-700">
              <fwb-radio v-model="catalogs.status" label="Sembunyikan" name="radio-bordered" value="hidden" />
            </div>
          </div>
        </div>
...
<script lang="ts" setup>
  ...
  const loadData = async () => {
    try {
      const req = await useAxios.get(`/user/catalog/${route.params.id}`)
      catalogs.value.description = req.data.description
      catalogs.value.keywords = req.data.keywords
      selectedType.value = req.data.type
      catalogs.value.name = req.data.name
      catalogs.value.status = req.data.status

      imagePreview1.value = req.data.image1Url
      imagePreview2.value = req.data.image2Url
      imagePreview3.value = req.data.image3Url

      categoryList.value = JSON.parse(req.data.categories ?? "[]");
      newCatalogUrl.value = JSON.parse(req.data.catalogUrl ?? "[]");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { name, description, keywords, type, catalogUrl, categories, status } = catalogs.value

      if (!name || !description || !keywords || !categories || !type || !catalogUrl || !status) {      
  ...
  const updateCatalog = async () => {
    const { name, description, keywords, type, catalogUrl, categories, image1Url, image2Url, image3Url, status } = catalogs.value

    try {
      await useAxios.put(`/user/catalog/${route.params.id}`, {
        name, description, keywords, type, catalogUrl, categories, image1Url: image1Url || null, image2Url: image2Url || null, image3Url: image3Url || null, status
      })
